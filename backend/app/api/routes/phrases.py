...
def _out(phrase, known_map) -> PhraseOut:
    return PhraseOut(
        id=phrase.id,
...
async def _known_map(principal, db, phrases) -> dict[int, bool]:
    if principal.user_id is None:
        return {}
    return await phrases_repo.get_phrase_known_map(
        db, user_id=principal.user_id, phrase_ids=[p.id for p in phrases]
    )


@router.get("/templates", response_model=list[PhraseOut])
async def list_templates(
    ...
    """Reusable essay clichés, deduplicated and independent of any one prompt."""
    phrases = await phrases_repo.list_templates(db, level=level, essay_part=part)
