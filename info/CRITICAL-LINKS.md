...
2. **Cache-bust `?v=N`** — при смене CSS/JS обновлять версию в HTML
   ...
   `words-data.js` — `?v=2`, `pipeline.css` — `?v=7`, `enrich.js` — `?v=4`).
...
| # | phase | что чинит | размер (2026-07-17) |
|---|-------|-----------|---------------------|
| 1 | `repair_case` | омографы, склеенные фолдингом регистра | 1274 |
| 2 | `repair_ortho` | дореформенные написания, умершие в `failed` | 387 |
| 3 | `backfill` | обычная выдача (нетегированные слова) | 17 379 |
